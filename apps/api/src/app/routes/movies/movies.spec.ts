...
const putMovie = `${MOVIES_ROUTE}/${movieId}`;
...
describe(`get routes:`, () => {
  ...
  describe(`get ${MOVIES_ROUTE}`, () => {
    ...
    describe('&actor=', () => {
      ...
      it(`should return empty array when &year=wrongOne`, async () => {
    ...
    describe('&director=', () => {
      it(`should return correct result when &director=${validDirectorName}`, async () => {
        const res = await request(app).get(
          `${MOVIES_ROUTE}?director=${validDirectorName}`
        );
        expect(res.statusCode).toEqual(200);
        expect(res.body.length).toEqual(1);
        expect(
          isEveryElements(
            ...
            (currentMovieDirectorsList) =>
              currentMovieDirectorsList.includes(`${validDirectorName}`)
          )
        ).toEqual(true);
      });
